...
var L, R, h;
L = (scrButtonCheck("leftButton") || (global.directionalTapFix && scrButtonCheckPressed("leftButton")));
R = (scrButtonCheck("rightButton") || (global.directionalTapFix && scrButtonCheckPressed("rightButton")));
...
{
    if (scrButtonCheckPressed("shootButton"))
        scrPlayerShoot();
    if (scrButtonCheckPressed("jumpButton"))
        scrPlayerJump();
    if (scrButtonCheckReleased("jumpButton"))
        scrPlayerVJump();
    if (scrButtonCheckPressed("suicideButton"))
        scrKillPlayer();
}

if (global.adAlign && place_meeting(x,y+(global.grav),objBlock) && !frozen)  // A/D align
{
    if (scrButtonCheckPressed("alignLeftButton")) {hspeed -= 1;}
    if (scrButtonCheckPressed("alignRightButton")) {hspeed += 1;}
}
...
if (onVineL || onVineR)
{
    ...
    if (onVineL && scrButtonCheckPressed("rightButton")) || (onVineR && scrButtonCheckPressed("leftButton"))
    {
        if (scrButtonCheck("jumpButton"))  //jumping off vine
        {
            if (onVineR)
                hspeed = -15;
            else
...
applies_to=self
*/
if ((place_meeting(x,y,objPlayerKiller)) || ((x < 0 || x > room_width || y < 0 || y > room_height) && global.edgeDeath))  //check if player has left the room
    scrKillPlayer();
