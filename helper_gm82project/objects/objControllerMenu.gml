...
if (!settingButton)
{
    if (scrButtonCheckPressed("menuUpButton"))
    {
    ...
    else if (scrButtonCheckPressed("menuDownButton"))
    {
        sound_play(global.menuSound);
        select += 1;
        if(select > 11)
    ...
    else if (scrButtonCheckPressed("menuBackButton"))
    {
        scrSaveConfig();    //save changes
        instance_create(x,y,objOptionsMenu);
        instance_destroy();
    }
    else if (scrButtonCheckPressed("menuAcceptButton"))
    {
        if (select != 0)
        {
            if (select != 11)
    ...
    if (select == 0)
    {
        if (scrButtonCheckPressed("menuRightButton"))
        {
            global.controllerIndex += 1;
            if (global.controllerIndex > 1)
                global.controllerIndex = -1;
        }
        else if (scrButtonCheckPressed("menuLeftButton"))
        {
            global.controllerIndex -= 1;
            if (global.controllerIndex < -1)
                global.controllerIndex = 1;
...
if (global.controllerIndex != -1)
{
    ...
    var controllerText;
    controllerText = "Current controller: ";

    if (joystick_exists(global.controllerIndex))//say what type of controller is connected (i.e. Xbox, Playstation)
    {
        controllerText += joystick_name(global.controllerIndex);
    }
    else
    {
        controllerText += "None";
